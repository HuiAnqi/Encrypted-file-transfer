...
//验证网站：https://the-x.cn/cryptography/aes.aspx
...
int encrypt_ecb(unsigned char S_BOX[][16],string& mes){
...
//            	printf("%x ",B[i][j]);
	...
	char fileName[64];
	printf("请将密文写进文件，比如'test.txt':\n");
	if (scanf("%s", fileName) == 1) {
		writeStrToFile(result, len, fileName);
		printf("已经将密文写进%s中了,可以在当前目录中找到它。\n", fileName);
	}
...
int decrypt_ecb(unsigned char S_BOX[][16], unsigned char N_S_BOX[][16],string& mes){ 
